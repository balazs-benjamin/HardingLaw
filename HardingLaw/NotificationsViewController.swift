...
final class NotificationsViewController: JSQMessagesViewController {
    ...
    fileprivate lazy var storageRef: FIRStorageReference = FIRStorage.storage().reference(forURL: "gs://hardinglaw-159703.appspot.com/")
...
extension NotificationsViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    func imagePickerController(_ picker: UIImagePickerController,
        ...
        if let key = sendPhotoMessage() {
            ...
            self.storageRef.child(path).put(data as Data , metadata: nil) { (metadata, error) in
                if let error = error {
